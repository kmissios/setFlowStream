...
({
    ...
    { functionObjects::setFlow::modeType::VORTEX2DSTREAM, "vortex2DSTREAM" },
...
bool Foam::functionObjects::setFlow::execute()
{
    ...
    switch (mode_)
    {
        ...
        case modeType::VORTEX2DSTREAM:
        {
            ...
            // Note: R_ rotation not implemented in phi calculation
            const vectorField Cf(mesh_.Cf().primitiveField() - origin_);
            const scalarField xp(mesh_.points().component(0) - origin_.component(0));
            const scalarField yp(mesh_.points().component(1) - origin_.component(1));
            const scalarField zp(mesh_.points().component(2) - origin_.component(2));
            const vectorField Cp(mesh_.points() - origin_);
            // const scalarField Xf(Cf.component(vector::X));
            // const scalarField Yf(Cf.component(vector::Y));
            // const scalarField Zf(Cf.component(vector::Z));
            // vectorField Uf(Xf.size());
            ...
            vectorField psi_v(xp.size());
            psi_v.replace(0, 0.0);
            psi_v.replace(1, (1.0/pi)*sqr(sin(pi*xp))*sqr(sin(pi*zp)));
            psi_v.replace(2, 0.0);
            ...
            forAll(phic, fi)
            {
                ...
                forAll(f, fpi)
                {
                    ...
                    phic[fi] += 0.5*(psi_v[p1] + psi_v[p2])&(Cp[p2] - Cp[p1]);
                }
            }
            // const vectorField& Sfc = mesh_.Sf().primitiveField();
            // phic = Uf & Sfc;
